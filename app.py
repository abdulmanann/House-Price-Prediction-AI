...
st.sidebar.image(
    Image, caption="Project for Artificial Intelligence", use_column_width=True
)
...
if choices == "Price Prediction":
    ...
    Baths = st.number_input(
        "Enter number of Bathrooms", min_value=1, max_value=11, format="%d"
    )
    Bedrooms = st.number_input(
        "Enter number of Rooms", min_value=1, max_value=11, format="%d"
    )
    Area = st.number_input(
        "Enter Area of House (in marla)",
        step=1.0,
        min_value=1.0,
        max_value=100.0,
        format="%.1f",
    ...
    df_dummy = pd.get_dummies(df, columns=["Property_Type", "City", "Area_Name"])
...
if choices == "About":
    ...
    info = """
        ...
        - Choosing a machine learning model: we selected neural networks as ML model that was suitable for the problem of predicting house prices.
